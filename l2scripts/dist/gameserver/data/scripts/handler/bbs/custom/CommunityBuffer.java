...
public class CommunityBuffer extends ScriptsCommunityHandler
{
	...
	public void onInit()
	{
		...
		for(int[] buff : BBSConfig.BUFF_SERVICE_AVAILABLE_SKILLS_FOR_BUFF)
		{
			...
			int enchantGroup = buff.length >= 3 ? buff[2] : 0;
			if(enchantGroup > 0)
			{
				lvl = SkillUtils.getSkillLevelMask(lvl, SkillUtils.getSubSkillLevel(enchantGroup, skill.getEnchantLevelsCount()));
				Skill enchantedSkill = SkillHolder.getInstance().getSkill(id, lvl);
