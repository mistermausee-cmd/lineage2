 ...
 * @author pchayka
 */
public class GvG implements OnInitScriptListener
{
	...
	public static class Countdown extends RunnableImpl
	{
		...
		{
			if(isLangRus)
				Announcements.announceToAll("GvG: До конца приема заявок на турнир осталось " + Integer.toString(_timer) + " мин.");
			
			else
				Announcements.announceToAll("GvG: Until the end of the registration remains " + Integer.toString(_timer) + " minutes.");
		}
	}
	...
	@Bypass("events.GvG.GvG:activateEventGMPanel")
	public void activateEventGMPanel(Player player, NpcInstance npc, String[] param)
	{
	...
	public static void activateEvent()
	{
		if(!isActive() && canBeStarted())
		{
			...
			if(isLangRus)
			{
				Announcements.announceToAll("Регистрация на GvG турнир началась! Community Board(Alt+B) -> Эвенты -> GvG (регистрация группы, описание)");
				Announcements.announceToAll("Заявки принимаются в течение " + regActiveTime / 60000 + " минут");
			}
			else
			{
				Announcements.announceToAll("The registration for GvG event is open! Community Board(Alt+B) -> Events -> GvG (group registration, overview)");
				Announcements.announceToAll("The registration will be open for " + regActiveTime / 60000 + " minutes");
			}	
			_active = true;
	...
	public static void deactivateEvent()
	{
		if(isActive())
		{
			...
			if(isLangRus)
				Announcements.announceToAll("GvG: Турнир отменен");
			else
				Announcements.announceToAll("GvG: The event is canceled");
			_active = false;
	 ...
	 */
	@Bypass("events.GvG.GvG:showStats")
	public void showStats(Player player, NpcInstance npc, String[] param)
	{
		if(!player.getPlayerAccess().IsEventGm)
			return;
		...
		String refresh = "<button value=\"Refresh\" action=\"bypass -h htmbypass_events.GvG.GvG:showStats\" width=60 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">";
		String start = "<button value=\"Start Now\" action=\"bypass -h htmbypass_events.GvG.GvG:startNow\" width=60 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">";
		int i = 0;
		...
		if(!leaderList.isEmpty())
		{
			...
			Functions.show("There are " + i + " group leaders who registered for the event:\n\n" + string + "\n\n" + refresh + "\n\n" + start, player, null);
		}
		else
			Functions.show("There are no participants at the time\n\n" + refresh, player, null);
	}

	@Bypass("events.GvG.GvG:startNow")
	public void startNow(Player player, NpcInstance npc, String[] param)
	{
		if(!player.getPlayerAccess().IsEventGm)
			return;
	 ...
	 */
	@Bypass("events.GvG.GvG:addGroup")
	public void addGroup(Player player, NpcInstance npc, String[] param)
	{
	...
	private static void prepare()
	{
		...
		if(leaderList.size() < 2)
		{
			...
			if(isLangRus)
			{
				Announcements.announceToAll("GvG: Турнир отменен из-за недостатка участников");
				return;
			}
			else
			{
				Announcements.announceToAll("GvG: The event is canceled due lack of perticipants.");
				return;
			}			
		}
		if(isLangRus)
			Announcements.announceToAll("GvG: Прием заявок завершен. Запуск турнира.");
			
		else
			Announcements.announceToAll("GvG: Registration is closed, starting the event!");
		start();
	}
