				...
				case 2:
					switch(step_stage2)
					{
						...
						case 3:
							...
							actor.addStatFunc(new FuncMul(Stats.INFLICTS_P_DAMAGE_POWER, 0x40, actor, 10));
							actor.addStatFunc(new FuncMul(Stats.INFLICTS_M_DAMAGE_POWER, 0x40, actor, 10));
							actor.addStatFunc(new FuncMul(Stats.RUN_SPEED, 0x40, actor, 3));
							actor.addSkill(SkillHolder.getInstance().getSkillEntry(1467, 1));
							actor.sendChanges();
							step_stage2 = 4;
							return true;
						case 4:
	...
	protected boolean createNewTask()
	{
		...
		for(NpcInstance npc : World.getAroundNpc(actor, 1000, 200))
			if(Rnd.chance(10) && npc != null && npc.getNpcId() == 20235)
			{
				...
				if(Rnd.chance(20))
				{
					SkillEntry skillEntry = actor.getKnownSkill(1467);
					if(skillEntry != null)
						addTaskCast(monster, skillEntry.getTemplate());
				}
				else
