...
public class _195_SevenSignsSecretRitualofthePriests extends Quest
{
	...
	private static final int EXP_REWARD = 10000000;	private static final int SP_REWARD = 2400; 	public _195_SevenSignsSecretRitualofthePriests()
	{
		...
		addLevelCheck("claudiaathebaldt_q195_0.htm", 79);	
		addQuestCompletedCheck("claudiaathebaldt_q195_0.htm", 194);			
	}
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("raymond_q195_3.htm"))
		{
			...
			SkillHolder.getInstance().getSkill(6204, 1).getEffects(player, player);
			st.setCond(3);
		}
		...
		{
			if(ref != null && player.getTransformId() == 113 && st.getQuestItemsCount(GuardsoftheDawnIdentityCard) >= 1)
			{
				...
				else
				{
					...
					player.startScenePlayer(SceneMovie.SSQ_SECRET_RITUAL_PRIEST);
					htmltext = "identityconfirmdevice_q195_1.htm";
				}
			}
			else
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		{
			if(cond == 3 && player.getTransformId() == 113)
			{
	...
	public void onEnterInstance(QuestState st, Reflection reflection, Object[] args)
	{
		...
		synchronized (guardsOfDawn1)
		{
			...
			{
				NpcInstance guard = reflection.addSpawnWithoutRespawn(18835, spawn[0], 0);
			...
			{
				NpcInstance guard = reflection.addSpawnWithoutRespawn(27352, spawn[0], 0);
				guard.setAI(new GuardofDawnFemale(guard, spawn[1]));
			}
			for(Location spawn[] : guardsOfDawn2)
			{
				NpcInstance guard = reflection.addSpawnWithoutRespawn(18834, spawn[0], 0);
				guard.setAI(new GuardofDawn(guard, spawn[1], spawn[2]));
			}
	...
	{
		for(Abnormal e : p.getAbnormalList())
