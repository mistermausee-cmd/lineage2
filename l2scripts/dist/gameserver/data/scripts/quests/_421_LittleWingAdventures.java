...
public class _421_LittleWingAdventures extends Quest
{
	...
	{
		PetInstance _pet = st.getPlayer().getPet();
		if(_pet == null)
			return false;
		if(CheckObjID)
		{
		...
		PetData template = PetDataHolder.getInstance().getTemplateByNpcId(_pet.getNpcId());
		if(template == null)
			return false;

		if(template.getControlItemId() != dragonflute.getItemId())
			return false;
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Mimyu)
		{
			...
			if(cond == 3)
			{
				...
				if(HatchlingSummoned(st, false) || st.getPlayer().hasServitor())
				...
				player.sendPacket(new InventoryUpdatePacket().addModifiedItem(player, dragonflute));
				st.finishQuest();
				return "30747_13.htm";
			}
		}
	...
	public class GuardiansSpawner extends RunnableImpl
	{
		...
		private List<String> agressors_pets = null;
		...
		public GuardiansSpawner(NpcInstance npc, QuestState st, int _count)
		{
			...
			for(int i = 0; i < _count; i++)
			{
				...
				agressors_pets = new ArrayList<String>();
				for(Servitor s : st.getPlayer().getServitors())
					agressors_pets.add(s.getName());
		...
		private void updateAgression()
		{
			...
			if(_player != null)
			{
				if(!agressors_pets.isEmpty())
				{
					for(String agressors_pet : agressors_pets)
					{
						for(Servitor s : _player.getServitors())
						{
							if(s.getName().equalsIgnoreCase(agressors_pet))
								AddAgression(s, 10);
						}
					}
				}
