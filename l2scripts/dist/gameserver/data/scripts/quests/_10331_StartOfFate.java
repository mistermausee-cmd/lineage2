...
import npc.model.InfiltrationOfficerInstance;
...
public class _10331_StartOfFate extends Quest
{
	...
	private static final String FIRST_ROOM_SPAWN_GROUP = "labyrinth_of_belis_q10331_1";	// Группа спавна в первой комнате.
	...
	public _10331_StartOfFate()
	{
		...
		addStartNpc(SEBION);
		addTalkId(FRANCO, RIVIAN, DEVON, TOOK, MOKA, VALFAR, INFILTRATION_OFFICER);
		...
		addLevelCheck("sebion_q10331_0.htm", 18);
		addClassLevelCheck("sebion_q10331_0.htm", false, ClassLevel.NONE);
		addRaceCheck("sebion_q10331_0a.htm", Race.HUMAN, Race.ELF, Race.DARKELF, Race.ORC, Race.DWARF, Race.KAMAEL);
	}
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("sebion_q10331_3.htm"))
		{
			st.setCond(1); // Ликвидация Немеритеса
		}
		...
		else if(event.equalsIgnoreCase("use_belise_mark"))
		{
			if(st.getInt("stage") == 3)
			{
				if(st.takeItems(BELIS_MARK, 1) == 1)
				{
					...
					if(marksLeft > 0)
					{
						htmltext = HtmCache.getInstance().getHtml("quests/_10331_StartOfFate/belis_verification_system_q10331_2.htm", st.getPlayer());
					...
					else
					{
						...
						if(reflect.getInstancedZoneId() == INSTANCED_ZONE_ID)
						{
							...
							if(officer != null)
								npcMoveToLocation(officer, new Location(-117896, 214248, -8617, 49151));
						}
		...
		else if(event.equalsIgnoreCase("start_stage_5"))
		{
			...
			if(reflect.getInstancedZoneId() == INSTANCED_ZONE_ID)
			{
				...
				if(generator != null)
				{
					...
					npcMoveToLocation(npc, generator.getLoc());
					npc.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION, generator, 1000000);
		...
		else if(event.equalsIgnoreCase("stage_5_spawn_timer"))
		{
			if(st.getInt("stage") == 5)
			{
				...
				if(defendersCount < NEED_DEFENDERS_KILLS_COUNT)
				{
					...
					if(officer != null)
					{
						if(officer instanceof InfiltrationOfficerInstance)
							((InfiltrationOfficerInstance) officer).setAttackableByMonsters(true);
						npcMoveToLocation(defender, officer.getLoc());
		...
		else if(event.equalsIgnoreCase("process_stage_5"))
		{
			...
			if(reflect.getInstancedZoneId() == INSTANCED_ZONE_ID)
			{
				...
				if(defenderKills >= NEED_DEFENDERS_KILLS_COUNT)
				{
					...
					{
						if(officer instanceof InfiltrationOfficerInstance)
							((InfiltrationOfficerInstance) officer).setAttackableByMonsters(false);
						npcMoveToLocation(officer, new Location(-119112, 213672, -8617, 8191));
					}
		...
		else if(event.equalsIgnoreCase("kill_nemertess"))
		{
			if(st.getInt("stage") == 7)
			{
				...
				if(reflect.getInstancedZoneId() == INSTANCED_ZONE_ID)
				{
					...
					st.setCond(2); // Ожерелье Сарила
					st.giveItems(SARILS_NECKLACE, 1); 
					NpcInstance officer = getNpcFromReflection(INFILTRATION_OFFICER, reflect);
					if(officer != null)
						npcMoveToLocation(officer, new Location(-118328, 212968, -8705, 24575));
				}
			}
			return null;
		}
		else if(event.startsWith("class_transfer"))
		{
			...
			htmltext = HtmCache.getInstance().getHtml("quests/_10331_StartOfFate/" + html, st.getPlayer());
			...
			st.giveItems(ADENA_ID, 14760);
			st.giveItems(46849, 1);
			st.giveItems(3948, 1500);
			st.giveItems(1463, 1500);
			st.giveItems(736, 20);
			st.addExpAndSp(296000, 15);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		{
			if(cond == 3)
				htmltext = getAvailableClassList(st.getPlayer(), "high_priest_franco_q10331_1.htm", "high_priest_franco_q10331_2.htm");
		}
		else if(npcId == RIVIAN)
		{
			if(cond == 4)
				htmltext = getAvailableClassList(st.getPlayer(), "grand_master_rivian_q10331_1.htm", "grand_master_rivian_q10331_2.htm");
		}
		else if(npcId == DEVON)
		{
			if(cond == 5)
				htmltext = getAvailableClassList(st.getPlayer(), "grand_magister_devon_q10331_1.htm", "grand_magister_devon_q10331_2.htm");
		}
		else if(npcId == TOOK)
		{
			if(cond == 6)
				htmltext = getAvailableClassList(st.getPlayer(), "high_prefect_took_q10331_1.htm", "high_prefect_took_q10331_2.htm");
		}
		else if(npcId == MOKA)
		{
			if(cond == 7)
				htmltext = getAvailableClassList(st.getPlayer(), "head_blacksmith_moka_q10331_1.htm", "head_blacksmith_moka_q10331_2.htm");
		}
		else if(npcId == VALFAR)
		{
			if(cond == 8)
				htmltext = getAvailableClassList(st.getPlayer(), "grand_master_valfar_q10331_1.htm", "grand_master_valfar_q10331_2.htm");
		}
		else if(npcId == SEBION)
		{
			...
			else if(cond == 2)
			{
				switch(st.getPlayer().getRace())
				{
					...
					case ELF:
					{
						htmltext = "sebion_q10331_5a.htm";
						st.setCond(4);
						break;
					}
					case DARKELF:
					{
						htmltext = "sebion_q10331_5b.htm";
						st.setCond(5);
						break;
					}
					case ORC:
					{
						htmltext = "sebion_q10331_5c.htm";
						st.setCond(6);
						break;
					}
					case DWARF:
					{
						htmltext = "sebion_q10331_5d.htm";
						st.setCond(7);
						break;
					}
					case KAMAEL:
					{
						htmltext = "sebion_q10331_5e.htm";
						st.setCond(8);
						break;
					}
				}
	...
	public String onFirstTalk(NpcInstance npc, Player player)
	{
		...
		if(reflect.getInstancedZoneId() == INSTANCED_ZONE_ID)
		{
			...
			else if(npcId == BELIS_VERIFICATION_SYSTEM)
			{
				...
				if(cond == 1 || cond == 2)
				{
					String htmltext = HtmCache.getInstance().getHtml("quests/_10331_StartOfFate/belis_verification_system_q10331_1.htm", st.getPlayer());
		...
		if(reflect.getInstancedZoneId() == INSTANCED_ZONE_ID)
		{
			...
			if(npcId == OPERATIVE)
			{
				if(st.getCond() == 1)
				{
					...
					{
						boolean heaveAliveNpcs = false;

						List<Spawner> spawners = reflect.getSpawners(FIRST_ROOM_SPAWN_GROUP);
						for(Spawner spawner : spawners)
						{
							for(NpcInstance spawnedNpc : spawner.getAllSpawned())
							{
						...
						if(!heaveAliveNpcs)
						{
							...
							NpcInstance officer = getNpcFromReflection(INFILTRATION_OFFICER, reflect);
							if(officer != null)
								npcMoveToLocation(officer, new Location(-117037, 212504, -8592, 39479));
						}
					}
	...
	{
		int npcId = npc.getNpcId();
		Race race = st.getPlayer().getRace();
		switch(npcId)
		{
			case FRANCO:
				if(race == Race.HUMAN)
	...
	private static void npcMoveToLocation(NpcInstance npc, Location loc)
	{
		...
		npc.setRunning(); // На всякий пожарный.
		npc.setSpawnedLoc(loc);
		if(!npc.moveToLocation(loc, 0, true))
		{
			npc.getAI().clientStopMoving();
	...
	{
		String htmltext = HtmCache.getInstance().getHtml("quests/_10331_StartOfFate/" + html, player);
		...
		for(ClassId firstClassId : ClassId.VALUES)
		{
			...
			classList.append("<Button ALIGN=LEFT ICON=\"NORMAL\" action=\"bypass -h npc_%objectId%_QuestEvent 10331 class_transfer " + firstClsId + " " + html2 + "\">" + HtmlUtils.htmlClassName(firstClsId) + "</button>");
		}
