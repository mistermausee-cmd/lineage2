...
{
	//Glory: 100 Самоцветов для Аксессуаров (34992), 150 Обломков Доспехов(34991)
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(st.getQuestItemsCount(PART_ARM) < 70 || st.getQuestItemsCount(GEM_FOR_ACC) < 145)
				htmltext = "You don't have enough materials";
		}
		else if(event.equalsIgnoreCase("cat") || event.equalsIgnoreCase("racoon") || event.equalsIgnoreCase("rabbit"))
			if(st.getCond() == 8 && st.getQuestItemsCount(PART_ARM) >= 70 && st.getQuestItemsCount(GEM_FOR_ACC) >= 145)
			{
				st.takeItems(PART_ARM, 70);
				st.takeItems(GEM_FOR_ACC, 145);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == GENTLER)
			...
			else if(cond == 8 && (st.getQuestItemsCount(PART_ARM) < 70 || st.getQuestItemsCount(GEM_FOR_ACC) < 145))
