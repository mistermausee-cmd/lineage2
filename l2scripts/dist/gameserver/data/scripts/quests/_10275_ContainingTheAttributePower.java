...
public class _10275_ContainingTheAttributePower extends Quest
{
	...
	private static final int EXP_REWARD = 10000000;	private static final int SP_REWARD = 2400; 	public _10275_ContainingTheAttributePower()
	{
		...
		addLevelCheck(Holly, "30839-00.htm", 76);
		addLevelCheck(Weber, "31307-00.htm", 76);
	}
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.setCond(5);
			SkillHolder.getInstance().getSkill(2635, 1).getEffects(player, player);
			st.giveItems(YinSword, 1, Element.FIRE, 10);
		}
		else if(event.equalsIgnoreCase("32326-09.htm"))
		{
			st.setCond(10);
			SkillHolder.getInstance().getSkill(2636, 1).getEffects(player, player);
			st.giveItems(YangSword, 1, Element.EARTH, 10);
		}
		else
		{
	...
	public String onCompleted(NpcInstance npc, QuestState st)
	{
		...
		if(npc.getNpcId() == Holly)
			htmltext = "30839-0a.htm";
		else if(npc.getNpcId() == Weber)
