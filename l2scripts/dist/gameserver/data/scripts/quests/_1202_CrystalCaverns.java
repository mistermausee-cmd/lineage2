...
public class _1202_CrystalCaverns extends Quest
{
		...
		else if(event.equalsIgnoreCase("out"))
		{
			if(player.getParty() != null)
			{
				...
				for(Player pl : player.getParty().getPartyMembers())
					pl.teleToLocation(149361, 172327, -945, ReflectionManager.MAIN);
			}
			else
				player.teleToLocation(149361, 172327, -945, ReflectionManager.MAIN);
			return null;
		}
		return event;
	}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		long seedsCount = (long) st.getPlayer().getRateItems();
		if(npcId == DARNEL)
		{
