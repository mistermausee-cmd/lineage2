...
public class HeartInfinityAttack extends Reflection
{
	...
	private static final int Ekimus = 29251;
	...
	public void notifyEchmusEntrance(Player leader)
	{
		...
		{
			for(Player p : getPlayers())
				p.startScenePlayer(SceneMovie.ECHMUS_OPENING);
	...
	private void handleEkimusStats()
	{
		...
		ekimus.addStatFunc(new FuncSet(Stats.POWER_ATTACK, 0x30, this, ekimus.getBaseStats().getPAtk() * 3)); // constant
		ekimus.addStatFunc(new FuncSet(Stats.MAGIC_ATTACK, 0x30, this, ekimus.getBaseStats().getMAtk() * 10)); // constant
		ekimus.addStatFunc(new FuncSet(Stats.POWER_DEFENCE, 0x30, this, ekimus.getBaseStats().getPDef() * a[1]));
		ekimus.addStatFunc(new FuncSet(Stats.MAGIC_DEFENCE, 0x30, this, ekimus.getBaseStats().getMDef() * a[0]));
		ekimus.addStatFunc(new FuncSet(Stats.REGENERATE_HP_RATE, 0x30, this, ekimus.getBaseStats().getHpReg() * a[2]));
	}
	...
	private void conquestConclusion(boolean win)
	{
		...
		for(Player p : getPlayers())
		{
			p.sendPacket(new SystemMessagePacket(SystemMsg.THIS_DUNGEON_WILL_EXPIRE_IN_S1_MINUTES).addInteger(15));
			...
			p.startScenePlayer(win ? SceneMovie.ECHMUS_SUCCESS : SceneMovie.ECHMUS_FAIL);
		}
		for(NpcInstance npc : getNpcs())
	...
	public void notifyEkimusIdle()
	{
		...
		ekimusIdleTask = ThreadPoolManager.getInstance().schedule(() ->
		{
			conquestConclusion(false);
		}, 180000L);
	}
