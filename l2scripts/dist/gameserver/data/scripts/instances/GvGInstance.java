...
import l2s.gameserver.network.l2.s2c.ExBlockUpSetList.RemovePlayer;
...
public class GvGInstance extends Reflection
{
	...
	public void start()
	{
		...
		final PointsInfo initialPoints = new PointsInfo(eventTime, team1Score, team2Score);
		final CloseUI cui = new CloseUI();
		...
		for(Player tm : HardReferences.unwrap(bothTeams))
		{
			...
			clientSetUp = new ChangePoints(eventTime, team1Score, team2Score, isRedTeam(tm), tm, 0);
	...
	private void end()
	{
		...
		final GameEnd end = new GameEnd(isRedWinner);
	...
	private void reward(Party party)
	{
		for(Player member : party.getPartyMembers())
		{
			...
			member.setFame(member.getFame() + 500, "GvG", true); // fame
	...
	private synchronized void changeScore(int teamId, int toAdd, int toSub, boolean subbing, boolean affectAnotherTeam, Player player)
	{
		...
		if(teamId == 1)
		{
			if(subbing)
			{
				...
				{
					team2Score += toAdd;
					broadCastPacketToBothTeams(new ChangePoints(timeLeft, team1Score, team2Score, true, player, getPlayerScore(player)));
				}
				broadCastPacketToBothTeams(new ChangePoints(timeLeft, team1Score, team2Score, false, player, getPlayerScore(player)));
			}
			else
			{
				team1Score += toAdd;
	...
	public void paralyzePlayers()
	{
		for(Player tm : HardReferences.unwrap(bothTeams))
		{
			...
			tm.getAbnormalList().stop(Skill.SKILL_MYSTIC_IMMUNITY);
			tm.block();
		}
	}
	...
	public void resurrectAtBase(Player player)
	{
		...
		player.altOnMagicUse(player, SkillHolder.getInstance().getSkill(5660, 2)); // Battlefield Death Syndrome
	...
	private void removePlayer(Player player, boolean legalQuit)
	{
		...
		broadCastPacketToBothTeams(new RemovePlayer(player, isRedTeam(player)));
		...
		player.leaveParty();
		if(!legalQuit)
			player.sendPacket(new GameEnd(false));
		player.teleToLocation(Location.findPointToStay(GvG.RETURN_LOC, 0, 150, ReflectionManager.MAIN.getGeoIndex()), ReflectionManager.MAIN);
	}
