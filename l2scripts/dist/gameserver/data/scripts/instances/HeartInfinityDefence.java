...
public class HeartInfinityDefence extends Reflection
{
	...
	private void conquestBegins()
	{
		...
		coffinSpawnTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(() ->
		{
			if(!conquestEnded)
				for(NpcInstance npc : getAllByNpcId(DeadTumor, true))
					spawnCoffin(npc);
		}, 1000L, 60000L);
		aliveTumorSpawnTask = ThreadPoolManager.getInstance().schedule(() ->
		{
			if(!conquestEnded)
			{
		...
		wagonSpawnTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(() -> addSpawnWithoutRespawn(SoulWagon, new Location(-179544, 207400, -15496), 0), 1000L, wagonRespawnTime);
