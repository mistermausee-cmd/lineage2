...
public class SufferingHallDefence extends Reflection
{
	...
	private void startDefence()
	{
		...
		coffinSpawnTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(() -> addSpawnWithoutRespawn(RegenerationCoffin, new Location(-173704, 218092, -9562, Location.getRandomHeading()), 250), 1000L, 10000L);
		monstersSpawnTask = ThreadPoolManager.getInstance().schedule(() -> spawnMonsters(), 60000L);
	}
	...
	private class DeathListener implements OnDeathListener
	{
		...
		public void onDeath(Creature self, Creature killer)
		{
			...
			if(ArrayUtils.contains(monsters, self.getNpcId()) && !checkAliveMonsters())
			{
				...
				monstersSpawnTask = ThreadPoolManager.getInstance().schedule(() -> spawnMonsters(), 40000L);
			}
