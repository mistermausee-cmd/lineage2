...
public class Frintezza extends Reflection
{
	...
	private ScheduledFuture<?> songEffectTask;
				...
				switch(_taskId)
				{
					...
					case 2:
						...
						for(int i = 0; i < 4; i++)
						{
							...
							portraits[i].getFlags().getImmobilized().start();
	...
	private class Music extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			Skill skill = SkillHolder.getInstance().getSkill(5007, song);
			if(skill != null)
			{
				if(!frintezza.isBlocked() && !frintezza.isActionsDisabled())
				{
					frintezza.broadcastPacket(new ExShowScreenMessage(song_name, 3000, ScreenMessageAlign.TOP_CENTER, true, 1, -1, true));
					frintezza.broadcastPacket(new MagicSkillUse(frintezza, skill.getId(), skill.getLevel(), skill.getHitTime(), 0));
					// Launch the song's effects (they start about 10 seconds after he starts to play)
					if(songEffectTask != null)
						songEffectTask.cancel(true);
					songEffectTask = ThreadPoolManager.getInstance().schedule(new SongEffectLaunched(getSongTargets(song), song), skill.getHitTime());
				}
				// Schedule a new song to be played in 30-40 seconds...
				musicTask = ThreadPoolManager.getInstance().schedule(new Music(), skill.getHitTime() + _intervalOfFrintezzaSongs + Rnd.get(10000));
			}
			else
			{
				// Schedule a new song to be played in 30-40 seconds...
	...
	private class SongEffectLaunched extends RunnableImpl
	{
		 ...
		 */
		public SongEffectLaunched(List<Creature> targets, int song)
		{
			_targets = targets;
			_song = song;
		}
		...
		public void runImpl() throws Exception
		{
			...
			if(frintezza.isBlocked() || frintezza.isActionsDisabled())
				return;
			frintezza.callSkill(SkillHolder.getInstance().getSkill(5008, _song), _targets, false, false);
		}
	}
	...
	private void blockUnblockNpcs(boolean block, int[] npcArray)
	{
		for(NpcInstance n : getNpcs())
			if(ArrayUtils.contains(npcArray, n.getNpcId()))
			{
				...
				{
					n.block();
					n.getFlags().getInvulnerable().start();
				}
				else
				{
					n.unblock();
					n.getFlags().getInvulnerable().stop();
				}
			}
	}
