...
public final class VoucherTraderInstance extends NpcInstance
{
	...
	public VoucherTraderInstance(int objectId, NpcTemplate template, MultiValueSet<String> set)
	{
	...
	public void onMenuSelect(Player player, int ask, long reply)
	{
		switch ((int) reply)
		{
			case 0:
				...
				{
					showChatWindow(player, "default/voucher_no_item.htm", false);
					return;
				}
				if (player.getVar(""+getNpcId()+"") != null && (Long.parseLong(player.getVar(""+getNpcId()+"")) > System.currentTimeMillis()))
				{
					showChatWindow(player, "default/voucher_time_left.htm", false);
					return;
				}

				player.setVar(""+getNpcId()+"", String.valueOf(System.currentTimeMillis() + 86400000L), -1);
				ItemFunctions.deleteItem(player, sign, 1);
				ItemFunctions.addItem(player, seal, 20);
				player.addExpAndSp(exp, 0);
				showChatWindow(player, "default/"+getNpcId()+"-list.htm", false);
				return;
			case 1:
				switch (getNpcId())
				{
				...
				return;
			case 2:
				showChatWindow(player, "default/voucher_info.htm", false);
				return;
			case 3:
				...
				if (instance != null)
				{
					if(instance.getReturnLoc() != null)
						player.teleToLocation(instance.getReturnLoc(), ReflectionManager.MAIN);
					else
						player.setReflection(ReflectionManager.MAIN);
				}
				return;
			case 4:
				showChatWindow(player, "default/voucher_talisman_select.htm", false);
				return;
			case 11:
				showChatWindow(player, "default/voucher_talisman_select_class.htm", false);
				return;
			case 12:
				if(!player.getClassId().isAwaked())
				{
					showChatWindow(player, "default/voucher_talisman_no_class.htm", false);
					return;
				}
