	...
	public void onMenuSelect(Player player, int ask, long reply)
	{
		if(ask == 4378)
		{
			...
			if(reply == 1)
			{
				...
				if(player.isRegisteredInChaosFestival() || player.isChaosFestivalParticipant() || (player.getBlockCheckerArena() > -1)) // TODO underground coliseum and kratei checks.
					return;
				...
				final long trainingCampDuration = TrainingCampManager.getInstance().getTrainingCampDuration(player);
				...
				else if(player.hasPremiumAccount() || !Config.TRAINING_CAMP_PREMIUM_ONLY)
				{
					...
					trainingCamp = new TrainingCamp(player.getObjectId(), player.getActiveSubClass().getIndex(), player.getLevel(), System.currentTimeMillis());

					if(TrainingCampManager.getInstance().addTrainingCamp(player, trainingCamp))
					{
						final int timeRemaining = (int) (Config.TRAINING_CAMP_MAX_DURATION - Math.max(trainingCampDuration, 0));

						player.startTrainingCampTask(timeRemaining * 1000L);
						TrainingCampManager.getInstance().onEnterTrainingCamp(player);

						if(trainingCampDuration > 0)
							player.sendPacket(new ExTrainingZone_Admission(player.getLevel(), 0, (int) timeRemaining));
						else
			...
			else if(reply == 2)
			{
				...
				if(trainingCamp != null && trainingCamp.getObjectId() == player.getObjectId())
				{
					if(trainingCamp.getClassIndex() == player.getActiveSubClass().getIndex())
					{
						...
						if(trainingTime >= 1)
						{
							final long expGained = (long) ((trainingTime * (Experience.getExpForLevel(trainingCamp.getLevel()) * Experience.getTrainingRate(trainingCamp.getLevel()) * Config.RATE_XP_BY_LVL[trainingCamp.getLevel()])) / TrainingCamp.TRAINING_DIVIDER);
				...
				if(trainingCamp != null && trainingCamp.getObjectId() == player.getObjectId())
				{
					if(trainingCamp.getClassIndex() == player.getActiveSubClass().getIndex())
					{
						...
						if(trainingTime >= 1)
						{
							...
							TrainingCampManager.getInstance().setTrainingCampDuration(player, (int) trainingCamp.getTrainingTime(TimeUnit.SECONDS));
