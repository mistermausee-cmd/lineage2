...
public class FarmMessengerInstance extends NpcInstance
{
	...
	public FarmMessengerInstance(int objectId, NpcTemplate template, MultiValueSet<String> set)
	{
	...
	public void onBypassFeedback(final Player player, final String command)
	{
		...
		if(command.equalsIgnoreCase("registrationMenu"))
		{
			...
			showChatWindow(player, "residence2/clanhall/farm_kel_mahum_messenger_1.htm", false);
		}
		else if(command.equalsIgnoreCase("registerAsClan"))
		{
			...
			{
				showChatWindow(player, "residence2/clanhall/farm_kel_mahum_messenger_27.htm", false);
				return;
			}
		...
		else if(command.equalsIgnoreCase("registerAsMember"))
		{
			...
			if(siegeClan == null)
			{
				showChatWindow(player, "residence2/clanhall/farm_kel_mahum_messenger_7.htm", false);
				return;
			}
			if(siegeClan.getClan().getLeaderId() == player.getObjectId())
			{
				showChatWindow(player, "residence2/clanhall/farm_kel_mahum_messenger_5.htm", false);
				return;
			}

			if(siegeClan.getPlayers().contains(player.getObjectId()))
				showChatWindow(player, "residence2/clanhall/farm_kel_mahum_messenger_9.htm", false);
			else
			{
				if(siegeClan.getPlayers().size() >= 18)
				{
					showChatWindow(player, "residence2/clanhall/farm_kel_mahum_messenger_8.htm", false);
					return;
				}
		...
		else if(command.equalsIgnoreCase("setNpc"))
		{
			...
			showChatWindow(player, npcDialog(siegeClan), false);
		}
	...
	private boolean checkCond(Player player, boolean regMenu)
	{
		...
		{
			showChatWindow(player, "quests/_655_AGrandPlanForTamingWildBeasts/farm_messenger_q0655_11.htm", false, "%siege_time%",  TimeUtils.toSimpleFormat(clanHall.getSiegeDate()));
			return false;
		}
		...
		{
			showChatWindow(player, "quests/_655_AGrandPlanForTamingWildBeasts/farm_messenger_q0655_03.htm", false);
			return false;
		}
		...
		{
			showChatWindow(player, "quests/_655_AGrandPlanForTamingWildBeasts/farm_messenger_q0655_05.htm", false);
			return false;
		}

		if(clan.getHasHideout() == clanHall.getId())
		{
			showChatWindow(player, "residence2/clanhall/farm_kel_mahum_messenger_22.htm", false);
			return false;
		}

		if(clan.getHasHideout() != 0)
		{
			showChatWindow(player, "quests/_655_AGrandPlanForTamingWildBeasts/farm_messenger_q0655_04.htm", false);
			return false;
		}

		if(siegeClans.size() >= 5)
		{
			showChatWindow(player, "residence2/clanhall/farm_kel_mahum_messenger_21.htm", false);
			return false;
		}
	...
	public void showChatWindow(Player player, int val, boolean firstTalk, Object... arg)
	{
		...
		if(clan != null)
			showChatWindow(player, "residence2/clanhall/farm_messenger001.htm", firstTalk, "%owner_name%", clan.getName());
		else
			showChatWindow(player, "residence2/clanhall/farm_messenger002.htm", firstTalk);
	}
}
