...
public class GuardofDawnFemale extends DefaultAI
{
	...
	private static final SkillEntry _skillEntry = SkillHolder.getInstance().getSkillEntry(5978, 1);
	...
	private boolean checkAroundPlayers(NpcInstance actor)
	{
		for(Playable target : World.getAroundPlayables(actor, _aggrorange, _aggrorange))
			if(target != null && target.isPlayer() && !target.isSilentMoving() && !target.isInvulnerable() && GeoEngine.canSeeTarget(actor, target, false) && PositionUtils.isFacing(actor, target, 150))
			{
