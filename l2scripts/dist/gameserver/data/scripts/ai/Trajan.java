...
public class Trajan extends Fighter
{
	...
	{
		NpcInstance actor = getActor();
		if(skill.isOffensive() && Rnd.get() <= 0.1)
		{
			for(Player player : actor.getReflection().getPlayers())
	...
	protected void onEvtSpawn()
	{
		...
		_aggroTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(() ->
        {
			for(Player player : actor.getReflection().getPlayers())
		...
		_doomTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(() ->
        {
			for(Player player : actor.getReflection().getPlayers())
				if (PositionUtils.calculateDistance(player, actor, false) < 2500.)
				{
					if((Rnd.get() <= 0.1D) && (!actor.isAttackingNow()) && (!actor.isCastingNow()))
					{
						...
						actor.doCast(SkillHolder.getInstance().getSkillEntry(6268, 1), player, true);
					...
					else if (Rnd.get() <= 0.2D)
					{
						...
						doom.doCast(SkillHolder.getInstance().getSkillEntry(14113, 1), player, true);
