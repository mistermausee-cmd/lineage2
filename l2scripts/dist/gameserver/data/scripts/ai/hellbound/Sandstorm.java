...
public class Sandstorm extends DefaultAI
{
	...
	private static final SkillEntry SKILL1 = SkillHolder.getInstance().getSkillEntry(5435, 1);
	private static final SkillEntry SKILL2 = SkillHolder.getInstance().getSkillEntry(5494, 1);
	...
	protected boolean thinkActive()
	{
		...
		if(lastThrow + 5000 < System.currentTimeMillis())
			for(Playable target : World.getAroundPlayables(actor, AGGRO_RANGE, AGGRO_RANGE))
				if(target != null && !target.isAlikeDead() && !target.isInvulnerable() && target.isVisible() && GeoEngine.canSeeTarget(actor, target, false))
				{
