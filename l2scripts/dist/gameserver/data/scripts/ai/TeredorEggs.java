...
public class TeredorEggs extends Fighter
{
	...
	protected void thinkAttack()
	{
		if(!_activated)
		{
			if(actor.getAggroList().getMostHated(getMaxHateRange()) == null || !actor.getAggroList().getMostHated(getMaxHateRange()).isPlayer())
				return;
			Player player = (Player) actor.getAggroList().getMostHated(getMaxHateRange());
			...
			if(player.getParty() != null)
			{
				for (Playable playable : player.getParty().getPartyMembersWithPets()){
					...
					if(playable != null && actor.getDistance(playable.getLoc()) <= distanceToDebuff)
						actor.doCast(SkillHolder.getInstance().getSkillEntry(poisonId, poisonLevel), playable, true);
				}
			}
			_activated = true;
		}
