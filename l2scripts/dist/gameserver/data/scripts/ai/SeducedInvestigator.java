		...
		if(Rnd.chance(0.1) && _reuse + 30000 < System.currentTimeMillis())
		{
			...
			if(player.getReflectionId() == actor.getReflectionId())
			{
				...
				if(actor.getNpcId() == 36562)
					actor.doCast(SkillHolder.getInstance().getSkillEntry(buffs[0], 1), player, true);
				else if(actor.getNpcId() == 36563)
					actor.doCast(SkillHolder.getInstance().getSkillEntry(buffs[1], 1), player, true);
				else if(actor.getNpcId() == 36564)
					actor.doCast(SkillHolder.getInstance().getSkillEntry(buffs[2], 1), player, true);
				else
					actor.doCast(SkillHolder.getInstance().getSkillEntry(buffs[3], 1), player, true);
			}
		}
