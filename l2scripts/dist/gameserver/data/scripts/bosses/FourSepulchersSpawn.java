...
public class FourSepulchersSpawn
{
	...
	public static Map<Integer, List<NpcInstance>> _dukeMobs = new HashMap<Integer, List<NpcInstance>>();
	public static Map<Integer, List<NpcInstance>> _viscountMobs = new HashMap<Integer, List<NpcInstance>>();

	public static List<NpcInstance> _managers;
	...
	{
		_managers = new ArrayList<NpcInstance>();
		for(int i = 31921; i <= 31924; i++)
			try
			{
				...
				_managers.add(npc);
				_log.info("FourSepulchersManager: Spawned " + npc.getName());
			}
			catch(Exception e)
			{
				e.printStackTrace();
	...
	public static void spawnEmperorsGraveNpc(int npcId)
	{
		...
		if(monsterList != null)
		{
			for(NpcLocation loc : monsterList)
				_allMobs.add(NpcUtils.spawnSingle(loc.npcId, loc));
		}
	}
	...
	public static void spawnExecutionerOfHalisha(NpcInstance npc)
	{
		...
		_allMobs.add(NpcUtils.spawnSingle(_victim.get(npc.getNpcId()), npc.getLoc(), npc.getReflection()));
	}
	...
	public static void spawnKeyBox(NpcInstance npc)
	{
		...
		_allMobs.add(NpcUtils.spawnSingle(_keyBoxNpc.get(npc.getNpcId()), npc.getLoc(), npc.getReflection()));
	}
	...
	public static void spawnMonster(int npcId)
	{
		...
		List<NpcInstance> mobs = new ArrayList<NpcInstance>();
		...
		if(monsterList != null)
		{
			...
			for(NpcLocation loc : monsterList)
			{
				...
				NpcInstance npc = NpcUtils.spawnSingle(spawnKeyBoxMob ? 18149 : loc.npcId, loc);
				if(npc instanceof SepulcherMonsterInstance)
					((SepulcherMonsterInstance) npc).mysteriousBoxId = npcId;
				switch(npcId)
				{
	...
	public static void spawnMysteriousBox(int npcId)
	{
		...
		NpcInstance npc = NpcUtils.spawnSingle(_mysteriousBoxSpawns.get(npcId).npcId, _mysteriousBoxSpawns.get(npcId));
		_allMobs.add(npc);
	}

	public static synchronized boolean isDukeMobsAnnihilated(int npcId)
	{
		List<NpcInstance> mobs = _dukeMobs.get(npcId);
		if(mobs == null)
			return true;
	...
	{
		List<NpcInstance> mobs = _viscountMobs.get(npcId);
		if(mobs == null)
			return true;
		for(NpcInstance mob : mobs)
			if(!mob.isDead())
	...
	public static class GateKeeper extends Location
	{
		...
		{
			super(_x, _y, _z, _h);
			this.door = ReflectionUtils.getDoor(doorId);
			this.npcId = npcId;
			if(door == null)
