...
public class FourSepulchersManager implements OnInitScriptListener
{
	...
	private static void entry(int npcId, Player player)
	{
		...
		for(Player member : player.getParty().getPartyMembers())
		{
			...
			ItemFunctions.deleteItem(member, CHAPEL_KEY, 999999);
		}
	...
	public static void managerSay(int min)
	{
		...
		if(_inAttackTime)
		{
			...
			for(NpcInstance npc : FourSepulchersSpawn._managers)
			{
		...
		else if(_inEntryTime)
		{
			...
			for(NpcInstance npc : FourSepulchersSpawn._managers)
			{
				Functions.npcShout(npc, msg1);
				Functions.npcShout(npc, msg2);
			}
		}
	}
	...
	private static class ChangeAttackTime extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			{
				FourSepulchersSpawn._allMobs.add(NpcUtils.spawnSingle(gk.npcId, gk));
			}
