...
public class Skill extends StatTemplate implements Cloneable
{
	...
	public static final int SKILL_CRAFTING_MASTERY = 10312;
	...
	private final boolean _abnormalHasCombo;
	...
	public Skill(StatsSet set)
	{
		...
		_abnormalHasCombo = set.getBool("abnormal_has_combo", false);
			...
			if(pcTarget != null)
			{
				if(pcTarget != player)
				{
					...
					if(isOffensive())
					{
						...
						if(player.isInCtF())
						{
				          if(player.getTeam() != TeamType.NONE && pcTarget.getTeam() != TeamType.NONE && player.getTeam() == pcTarget.getTeam() && player != pcTarget) 
				                return SystemMsg.THAT_IS_AN_INCORRECT_TARGET;
				        }
	...
	{
		return _isCancelable && _isSelfDispellable && !hasEffect(EffectUseType.NORMAL, EffectType.Transformation) && !isToggle() && !isVitalityLimited() && !isAura();
	}
	...
	{
		if(_magicType == SkillMagicType.UNK_MAG_TYPE_21) 
			return true;
	...
	public void onEndCast(Creature activeChar, List<Creature> targets)
	{
		...
		if(!(isNotTargetAoE() && isOffensive() && targets.size() == 0))    
		{
	...
	public Skill[] getAnalogSkills(Player player)
	{
		...
		{
			SkillEntry analogSkill = player.getKnownSkill(analogId);
			if(analogSkill == null)
				continue;
	...
	public SkillEntry getElementalSkill(SkillEntry skillEntry, Player player)
	{
		...
		SkillEntry elementalSkillEntry = SkillHolder.getInstance().getSkillEntry(elementalSkillId, getLevel());
		if(elementalSkillEntry == null)
			return skillEntry;

		return elementalSkillEntry;
	}
	...
	public boolean isAbnormalHasCombo()
	{
	    return _abnormalHasCombo;
	}
