...
public abstract class Servitor extends Playable
{
	...
	protected void onDelete()
	{
		...
		if(isSummon())
		{
			...
			summons.forEach(summon -> owner.sendPacket(new MyPetSummonInfoPacket(summon)));
		}
	...
	public SkillEntry getAdditionalSSEffect(boolean spiritshot, boolean blessed)
	{
		if(!spiritshot)
		{
			...
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 47688, 0)) 
				return SkillHolder.getInstance().getSkillEntry(18716, 1);
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 38859, 0)) 
				return SkillHolder.getInstance().getSkillEntry(17891, 1);
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 38858, 0)) 
				return SkillHolder.getInstance().getSkillEntry(17890, 1);
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 38857, 0)) 
				return SkillHolder.getInstance().getSkillEntry(17889, 1);
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 38856, 0)) 
				return SkillHolder.getInstance().getSkillEntry(17888, 2);
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 38855, 0)) 
				return SkillHolder.getInstance().getSkillEntry(17888, 1);
		}
