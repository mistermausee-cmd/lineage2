...
public class Olympiad
{
	...
	public static synchronized boolean registerParticipant(Player player, CompType type)
	{
	    ...
	    if(player.isRegisteredInChaosFestival() || player.isChaosFestivalParticipant())
	    {
	    	player.sendPacket(SystemMsg.YOU_ARE_ON_THE_WAITING_LIST_FOR_THE_CEREMONY_OF_CHAOS);
	    	return false;
	    } 
	...
	{
		if(validPlayer.getLevel() < 85)
			return false;
		 
	    if(!validPlayer.getClassId().isAwaked())
	      return false; 
	...
	public static int convertParticipantClassId(int baseClassId)
	{
	    ...
	    if(!classId.isOfRace(Race.ERTHEIA))
	    {
	      if(!classId.isOfLevel(ClassLevel.AWAKED))
	      {
	        ...
	        {
	          if(!id.isOutdated() && id.isOfLevel(ClassLevel.AWAKED) && id.childOf(classId))
	            return id.getId();
	        }
	      }
	    }
	    else if(!classId.isOfLevel(ClassLevel.THIRD))
	    {
	      for(ClassId id : ClassId.VALUES)
	      {
	        if (id.isOfLevel(ClassLevel.THIRD) && id.childOf(classId))
	          return id.getId();
	      }
	    }
