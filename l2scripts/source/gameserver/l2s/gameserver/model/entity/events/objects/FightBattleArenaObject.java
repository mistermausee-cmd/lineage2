...
public final class FightBattleArenaObject extends ObservableArena implements Serializable, Comparable<FightBattleArenaObject>
{
		...
		public void runImpl()
		{
			...
			if(isOpponentSearching())
			{
				...
				for(FightBattlePlayerObject member : getMembers())
				{
					...
					String text = new CustomMessage("l2s.gameserver.model.entity.events.objects.FightBattleArenaObject.0.0").toString(player);
				...
				switch(_taskTemp)
				{
					...
					case 32:
						for(FightBattlePlayerObject member : getMembers())
						{
							...
							String text = new CustomMessage("l2s.gameserver.model.entity.events.objects.FightBattleArenaObject.1").addNumber(33 - _taskTemp).toString(player);
    ...
    {
        return _info.getObserverLoc();
    }
    
    @Override
    public boolean showObservableArenasList(Player player)
    {
        player.sendPacket(new ExReceiveOlympiadPacket.MatchList(getEvent()));
        return true;
    }
	...
	private void startBattle()
	{
		...
		for(FightBattlePlayerObject member : getMembers())
		{
			...
			String text = new CustomMessage("l2s.gameserver.model.entity.events.objects.FightBattleArenaObject.2").toString(player);
			player.sendPacket(new ExShowScreenMessage(text, 5000, ScreenMessageAlign.TOP_CENTER, true));
			player.sendMessage(text);
	...
	private void broadcastPacket(IBroadcastPacket packet, FightBattlePlayerObject exception, boolean withObservers)
	{
		for(FightBattlePlayerObject member : getMembers())
		{
			if(member == null || member == exception)
	...
	private void broadcastPacket(IBroadcastPacket packet, boolean withObservers)
	{
		broadcastPacket(packet, null, withObservers);
	}
