...
public class KrateisCubeEvent extends Event
{
	private class Listeners implements OnKillListener, OnPlayerExitListener, OnTeleportListener
	{
		...
		public void onKill(Creature actor, Creature victim)
		{
			...
			if(!victim.containsEvent(KrateisCubeEvent.this))
		        return;
			...
			KrateisCubePlayerObject looserPlayer = KrateisCubeEvent.this.getParticlePlayer((Player)victim);
	...
	public void prepare()
	{
		...
		for(KrateisCubePlayerObject k : registeredPlayers)
		{
			...
			player.teleToLocation(Rnd.get(waitLocs), ReflectionManager.MAIN);
		}
	}
	...
	public void announce(SystemMsg msgId, int a, int time)
	{
		...
		if(a > 0)
			p = new SystemMessagePacket(SystemMsg.S1_SECONDS_TO_GAME_END).addInteger(a);
		else
			p = new SystemMessagePacket(SystemMsg.THE_MATCH_WILL_START_IN_S1_SECONDS).addInteger(-a);
