...
public class MonsterInstance extends NpcInstance
{
	...
	private double[] calculateExpAndSp(int level, long damage)
	{
		...
		if(Math.abs(diff) > Config.MIN_DIFF_LEVELS_FOR_EXP_SP_PENALTY)
            return new double[] { 0.0, 0.0 };

        if(level > 77 && diff > 3 && diff <= 5)
            diff += 3;

        double xp = SafeMath.mulAndLimit(getExpReward(), damage / (double) getMaxHp());
        double sp = SafeMath.mulAndLimit(getSpReward(), damage / (double) getMaxHp());
        ...
        if(level > 84 && diff < -2)
        {
            double mod = Math.pow(0.83, Math.abs(diff + 3));
            xp = SafeMath.mulAndLimit(xp, mod);
            sp = SafeMath.mulAndLimit(sp, mod);
        }
    ...
    private void calculateEnergyofDestruction(Creature killer)
    {
        ...
        if(player.getDestructionCount() >= 2)
            return;

        int count = 1;
        if(player.getDestructionCount() == 0 && Rnd.chance(Config.ENERGY_OF_DESTRUCTION_DOUBLE_CHANCE))
            count = 2;

        player.setDestructionCount(player.getDestructionCount() + count);

        if(player.getDestructionCount() == 1)
        	player.sendPacket(SystemMsg.YOU_HAVE_OBTAINED_THE_FIRST_ENERGY_OF_DESTRUCTION_YOU_CAN_OBTAIN_UP_TO_2_OF_THESE_A_DAY_AND_CAN_BEGIN_OBTAINING_THEM_AGAIN_AT_630AM_EVERY_DAY);
        else
        	player.sendPacket(SystemMsg.YOU_HAVE_OBTAINED_THE_SECOND_ENERGY_OF_DESTRUCTION_YOU_CAN_OBTAIN_UP_TO_2_OF_THESE_A_DAY_AND_CAN_BEGIN_OBTAINING_THEM_AGAIN_AT_630AM_EVERY_DAY);
        
        ItemFunctions.addItem(player, 35562, count); 
    }
    ...
    public void calculateMarkEndurity(Creature killer)
    {
        ...
        ItemFunctions.addItem(player, 17742, 1); 
    }
