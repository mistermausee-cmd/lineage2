...
public class VillageMasterPledgeBypasses
{
	...
	public static void levelUpClan(NpcInstance npc, Player player, boolean alternative)
	{
		...
		if(alternative && Config.ALTERNATIVE_CLAN_LVL_UP_RP_COST[nextClanLevel] == -1)
		{
			npc.showChatWindow(player, "pledge/pledge_pl001.htm", false, new Object[0]);
			return;
		}
		final int spCost = alternative ? 0 : Config.CLAN_LVL_UP_SP_COST[nextClanLevel];
		final int rpCost = alternative ? Config.ALTERNATIVE_CLAN_LVL_UP_RP_COST[nextClanLevel] : Config.CLAN_LVL_UP_RP_COST[nextClanLevel];
		final int minMembers = Config.CLAN_LVL_UP_MIN_MEMBERS[nextClanLevel];
		final long[][][] itemsRequiredVariations = alternative ? new long[0][][] : Config.CLAN_LVL_UP_ITEMS_REQUIRED[nextClanLevel];
		final boolean needCastle = alternative ? false : Config.CLAN_LVL_UP_NEED_CASTLE[nextClanLevel];
	...
	protected static void renameSubPledgeCheck(NpcInstance npc, Player player, String command)
	{
		...
		if(subUnit == null)
		{
			npc.showChatWindow(player, dialogs[ERROR_NO_UNIT_RENAME_UNIT], false);
			return;
		}

		npc.showChatWindow(player, dialogs[START_RENAME_RENAME_UNIT], false);
	}
	...
	public static void renameSubPledge(NpcInstance npc, Player player, String command)
	{
		...
		for(SubUnit element : subPledge)
			...
			{
				npc.showChatWindow(player, "pledge/pl_already_subname.htm", false);
				return;
			}
		...
		npc.showChatWindow(player, dialogs[OK_RENAME_RENAME_UNIT], false);
	}
	...
	public static void upgradeSubPledge(NpcInstance npc, Player player, int subpledgeId)
	{
		...
		{
			npc.showChatWindow(player, "pledge/pl_upgrade_err_sub2.htm", false);
			return;
		}
		...
		npc.showChatWindow(player, "pledge/pl_upgrade_ok_sub2.htm", false);
	}
	...
	public static boolean createSubPledge(NpcInstance npc, Player player, String clanName, final int pledgeType, int minClanLvl, String leaderName)
	{
		...
		if(pledgeType != Clan.SUBUNIT_ACADEMY)
		{
			...
			if(subLeader == null)
			{
				player.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2VillageMasterInstance.PlayerCantBeAssignedAsSubUnitLeader"));
				return false;
			}
			else if(subLeader.isLeaderOf() != Clan.SUBUNIT_NONE)
			{
				player.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2VillageMasterInstance.ItCantBeSubUnitLeader"));
				return false;
			}
		}
		...
		if(result == Clan.SUBUNIT_NONE)
		{
			switch(pledgeType)
			{
				case Clan.SUBUNIT_ACADEMY:
					...
					break;
				case Clan.SUBUNIT_ROYAL1:
					npc.showChatWindow(player, "pledge/pl_err_more_sub.htm", false);
					break;
				case Clan.SUBUNIT_KNIGHT1:
					npc.showChatWindow(player, "pledge/pl_err_more_sub.htm", false);
					break;
	...
	public static boolean changeLeader(NpcInstance npc, Player player, int pledgeId, String leaderName)
	{
		...
		else
		{
			...
			npc.showChatWindow(player, "pledge/pl_create_ok_submaster.htm", false);
		}
		return true;
	}
