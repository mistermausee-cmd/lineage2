...
public class NpcInstance extends Creature
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		try
		{
			...
			else if(command.startsWith("EnterRift"))
			{
				...
				Integer b1 = Integer.parseInt(st2.nextToken()); 
			...
			else if(cmd.equalsIgnoreCase("teleport"))
			{
				...
				else if(cmd2.equalsIgnoreCase("noble"))
				{
					if(player.isNoble() || Config.ALLOW_NOBLE_TP_TO_ALL)
						showChatWindow(player, "common/fornobless.htm", false);
					else
						showChatWindow(player, "common/fornonobless.htm", false);
				}
			...
			else if(command.startsWith("RemoveTransferSkill"))
			{
				...
				{
					showChatWindow(player, "common/skill_share_healer_no_adena.htm", false);
					return;
				}
				...
				for(SkillLearn skill : skills)
					if(player.removeSkill(skill.getId(), true) != null)
						for (ItemData item : skill.getRequiredItemsForLearn(type))
							ItemFunctions.addItem(player, item.getId(), item.getCount(), true); 
			}
			...
			if(ItemFunctions.getItemCount(player, itemId) < itemCount)
			{
				if(itemId == ItemTemplate.ITEM_ID_ADENA)
					player.sendPacket(SystemMsg.YOU_DO_NOT_HAVE_ENOUGH_ADENA);
			          ...
			          showChatWindow(player, "common/fornonoblessitem.htm", false);
				else
	...
	public void showTransformationMultisell(Player player)
	{
		if(!Config.ALLOW_LEARN_TRANS_SKILLS_WO_QUEST)
			...
			{
				showChatWindow(player, "default/" + getNpcId() + "-nobuy.htm", false);
				return;
			}

		Castle castle = getCastle(player);
		MultiSellHolder.getInstance().SeparateAndSend(32323, player, castle != null ? castle.getSellTaxRate() : 0);
		player.sendActionFailed();
	}
	...
	public void showTransformationSkillList(Player player, AcquireType type)
	{
		if(!Config.ALLOW_LEARN_TRANS_SKILLS_WO_QUEST)
			if(!player.isQuestCompleted(136))
			{
				showChatWindow(player, "default/" + getNpcId() + "-noquest.htm", false);
				return;
			}
	...
	public static void showAlchemySkillList(Player player)
	{
	...
	{
		return packet == RequestRefine.class || packet == RequestRefineCancel.class || packet == RequestItemEnsoul.class || packet == RequestTryEnSoulExtraction.class;
	}
