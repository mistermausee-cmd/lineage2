...
public class Party implements PlayerGroup
{
	...
	public boolean removePartyMember(Player player, boolean kick)
	{
		if(getMemberCount() == MAX_SIZE)
		{
		      ...
		      {
		        WorldStatisticsManager.getInstance().updateStat(player, CategoryType.TIME_IN_FULLPARTY, (System.currentTimeMillis() - player.getStartingTimeInFullParty()) / 1000L);
		...
		WorldStatisticsManager.getInstance().updateStat(player, CategoryType.TIME_IN_PARTY, (System.currentTimeMillis() - player.getStartingTimeInParty()) / 1000L);
	...
	private void updateLeaderInfo()
	{
		...
		for(Player member : _members)
		{
			...
			member.sendPacket(PartySmallWindowDeleteAllPacket.STATIC, 
					new PartySmallWindowAllPacket(this, member), 
					msg); 
		}
