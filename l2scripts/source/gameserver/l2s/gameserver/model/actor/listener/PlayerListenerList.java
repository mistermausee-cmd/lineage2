...
public class PlayerListenerList extends CharListenerList
{
	...
	public void onChaosFestivalFinishBattle(boolean winner)
	{
		if(!global.getListeners().isEmpty())
			for(Listener<Creature> listener : global.getListeners())
				if(OnChaosFestivalFinishBattleListener.class.isInstance(listener))
					((OnChaosFestivalFinishBattleListener) listener).onChaosFestivalFinishBattle(getActor(), winner);
