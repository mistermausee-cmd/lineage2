...
public class PlayerBaseStats extends PlayableBaseStats
{
	...
	public double getPDef()
	{
		...
		double underwearPDef;
		if(getOwner().isTransformed() && getOwner().getTransform().getBaseUnderwearDef() > 0)
			underwearPDef = getOwner().getTransform().getBaseUnderwearDef();
		else
			underwearPDef = getOwner().getTemplate().getBaseUnderwearDef();
		
		tempItem = getOwner().getInventory().getPaperdollItem(Inventory.PAPERDOLL_UNDER);
		if(tempItem != null)
			underwearPDef = Math.max(underwearPDef, tempItem.getTemplate().getPDef());

		result += underwearPDef;
