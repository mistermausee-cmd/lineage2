...
public class SubClassList
{
	...
	private SubClass _dualSubClass = null;
	...
	{
		return _dualSubClass;
	}

	public void setDualSubClass(SubClass sub)
	{
		_dualSubClass = sub;
	}
		...
		if(sub.isBase())
		{
			if(ClassId.VALUES[newClassId].isOfLevel(ClassLevel.AWAKED))
				if(!ClassId.VALUES[oldClassId].isOfLevel(ClassLevel.AWAKED))
					sub.setDefaultClassId(oldClassId);
		}
		else
			sub.setDefaultClassId(ClassId.VALUES[newClassId].getBaseAwakeParent(ClassId.VALUES[oldClassId]).getId());
