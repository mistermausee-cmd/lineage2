...
import l2s.gameserver.network.l2.s2c.ExCubeGameChangeTeam;
...
public final class HandysBlockCheckerManager
{
	...
	public boolean addPlayerToArena(Player player, int arenaId)
	{
		...
		synchronized (holder)
		{
			...
			KrateisCubeRunnerEvent krateis = EventHolder.getInstance().getEvent(EventType.MAIN_EVENT, 1);
	...
	public void removePlayer(Player player, int arenaId, int team)
	{
		...
		synchronized (holder)
		{
			...
			holder.broadCastPacketToTeam(new RemovePlayer(player, isRed));
