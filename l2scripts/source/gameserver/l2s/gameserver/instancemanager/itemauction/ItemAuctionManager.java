...
public class ItemAuctionManager
{
	...
	public static final int CUSTOM_AUCTION_MANAGER_ID = -1;
	...
	private void load()
	{
		...
		try
		{
			...
			if(Config.ALT_CUSTOM_ITEM_AUCTION_ENABLED)
			{
				...
				statement = con.prepareStatement("SELECT * FROM custom_item_auction ORDER BY auctionId");
				rset = statement.executeQuery();
				while(rset.next())
				{
					int auctionId = rset.getInt("auctionId");
					int auctionItemId = rset.getInt("auctionItemId");
					int itemId = rset.getInt("itemId");
					long itemCount = rset.getLong("itemCount");
					int itemEnchant = rset.getInt("itemEnchant");
					int bidItemId = rset.getInt("bidItemId");
					long bidItemStartCount = rset.getLong("bidItemStartCount");

					AuctionItem item = new AuctionItem(auctionItemId, 0, bidItemId, bidItemStartCount, itemId, itemCount);
					item.setEnchantLevel(itemEnchant);
				...
				try
				{
					ItemAuctionInstance instance = new ItemAuctionInstance(CUSTOM_AUCTION_MANAGER_ID, new SchedulingPattern("* * * * *"), items);
					_managerInstances.put(CUSTOM_AUCTION_MANAGER_ID, instance);
				}
				catch(Exception e)
				{
					_log.error("ItemAuctionManager: Error while loading custom auctions!", e);
				}
			}
		}
	...
	public void deleteAuction(int auctionId)
	{
		...
		try
		{
			...
			statement = con.prepareStatement("DELETE FROM custom_item_auction WHERE auctionId=?");
			statement.setInt(1, auctionId);
			statement.execute();
		}
		catch(SQLException e)
