...
public class CursedWeaponsManager
{
						...
						if("item".equalsIgnoreCase(d.getNodeName()))
						{
							...
							for(Node cd = d.getFirstChild(); cd != null; cd = cd.getNextSibling())
								...
								else if("duration".equalsIgnoreCase(cd.getNodeName()))
								{
									...
									cw.setTransformId(Integer.parseInt(cd.getAttributes().getNamedItem("val").getNodeValue()));
	...
	public void endOfLife(CursedWeapon cw)
	{
		if(cw.isActivated())
		{
			...
			if(player != null)
			{
				...
				player.removeSkill(SkillHolder.getInstance().getSkillEntry(cw.getSkillId(), player.getSkillLevel(cw.getSkillId())), false);
	...
	{
		if(killer.isInOlympiadMode() || killer.isCursedWeaponEquipped() || _cursedWeapons.length == 0 || !killer.getReflection().isMain())
			return;
