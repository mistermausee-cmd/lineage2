...
public class CommissionManager
{
	...
	public boolean addCommissionItem(CommissionItem item)
	{
		...
		try
		{
			if(containsCommissionItem(item.getCommissionId())) 
				return false;
	...
	public IBroadcastPacket registerItem(Player player, int itemObjId, long itemPrice, long itemCount, int periodDays)
	{
		writeLock();
		try
		{
			...
			try
			{
				...
				if((itemPrice * itemCount) <= CommissionUtils.getCommissionTax(itemPrice, itemCount, periodDays))
					return SystemMsg.THE_ITEM_CANNOT_BE_REGISTERED_BECAUSE_REQUIREMENTS_ARE_NOT_MET;
				...
				if(!ItemFunctions.deleteItem(player, item, itemCount, false)) 
				{
					deleteCommissionItem(commissionItem.getCommissionId());
					return SystemMsg.REGISTRATION_IS_NOT_AVAILABLE_BECAUSE_THE_CORRESPONDING_ITEM_DOES_NOT_EXIST;
				}
	...
	public IBroadcastPacket unregisterItem(Player player, int bidId)
	{
		writeLock();
		try
		{
	...
	public IBroadcastPacket buyItem(Player player, int bidId)
	{
		writeLock();
		try
		{
			...
			final long commissionPrice = commissionItem.getCommissionPrice() * commissionItem.getCount();
	...
	private CommissionItem[] getMyCommissionItems(int playerObjectId)
	{
		...
		Arrays.sort(itemsArray, (o1, o2) ->
		{
			return o1.getRegisterDate() - o2.getRegisterDate(); 
		});
		return itemsArray;
	}
	...
	private CommissionItem[] getCommissionItems(int tree, int type, int quality, int grade, String searchWords)
	{
		...
		Arrays.sort(itemsArray, (o1, o2) ->
		{
			return o2.getRegisterDate() - o1.getRegisterDate(); 
		});
		return itemsArray;
	}
	...
	public void sendMyCommissionList(Player player)
	{
		CommissionItem[] items = getMyCommissionItems(player.getObjectId());
		if(items.length > 0)
			player.sendPacket(new ExResponseCommissionList(ExResponseCommissionList.MY_COMMISSION_LIST, 0, items));
		else
			player.sendPacket(ExResponseCommissionList.MY_EMPTY_LIST_PACKET);
	}
	...
	{
		CommissionItem[] items = getCommissionItems(tree, type, quality, grade, searchWords);
		if(items.length > 0)
		{
			int part = 0;
			List<CommissionItem> list = new ArrayList<CommissionItem>();
			for(int i = 0; i < items.length; i++)
			{
				list.add(items[i]);
				if((list.size() % 120) == 0 || i == (items.length - 1))
				{
					player.sendPacket(new ExResponseCommissionList(ExResponseCommissionList.COMMON_COMMISSION_LIST, part, list.toArray(new CommissionItem[list.size()])));
					list.clear();
					part++;
				}
			}
			...
			player.sendPacket(ExResponseCommissionList.COMMON_EMPTY_LIST_PACKET);
	}
