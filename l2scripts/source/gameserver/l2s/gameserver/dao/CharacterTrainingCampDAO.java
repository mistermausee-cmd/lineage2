...
public class CharacterTrainingCampDAO
{
	...
	public void restore(Map<String, TrainingCamp> map)
	{
		...
		try
		{
			...
			while(rset.next())
				map.put(rset.getString("account_name"), new TrainingCamp(rset.getInt("char_id"), rset.getInt("class_index"), rset.getInt("level"), rset.getLong("start_time") * 1000L, rset.getLong("end_time") * 1000L));
		}
		catch(Exception e)
		{
