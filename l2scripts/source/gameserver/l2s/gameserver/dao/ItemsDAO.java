...
public class ItemsDAO implements JdbcDAO<Integer, ItemInstance>
{
	...
	private final static String RESTORE_ITEM = "SELECT object_id, owner_id, item_id, count, enchant_level, loc, loc_data, custom_type1, custom_type2, life_time, custom_flags, variation_stone_id, variation1_id, variation2_id, attribute_fire, attribute_water, attribute_wind, attribute_earth, attribute_holy, attribute_unholy, agathion_energy, appearance_stone_id, visual_id FROM items WHERE object_id = ?";
	private final static String RESTORE_OWNER_ITEMS = "SELECT object_id FROM items WHERE owner_id = ? AND loc = ?";
	private final static String STORE_ITEM = "INSERT INTO items (object_id, owner_id, item_id, count, enchant_level, loc, loc_data, custom_type1, custom_type2, life_time, custom_flags, variation_stone_id, variation1_id, variation2_id, attribute_fire, attribute_water, attribute_wind, attribute_earth, attribute_holy, attribute_unholy, agathion_energy, appearance_stone_id, visual_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	private final static String UPDATE_ITEM = "UPDATE items SET owner_id = ?, item_id = ?, count = ?, enchant_level = ?, loc = ?, loc_data = ?, custom_type1 = ?, custom_type2 = ?, life_time = ?, custom_flags = ?, variation_stone_id = ?, variation1_id = ?, variation2_id = ?, attribute_fire = ?, attribute_water = ?, attribute_wind = ?, attribute_earth = ?, attribute_holy = ?, attribute_unholy = ?, agathion_energy=?, appearance_stone_id=?, visual_id=? WHERE object_id = ?";
	...
	private ItemInstance load0(ResultSet rset) throws SQLException
	{
		...
		if(rset.next())
		{
			...
			item.setAppearanceStoneId(rset.getInt(22));
			item.setVisualId(rset.getInt(23));
			item.restoreEnsoul();
		}
	...
	private void save0(ItemInstance item, PreparedStatement statement) throws SQLException
	{
	    ...
	    statement.setInt(22, item.getAppearanceStoneId());
	    statement.setInt(23, item.getVisualId());
	}
	...
	private void update0(ItemInstance item, PreparedStatement statement) throws SQLException
	{
	    ...
	    statement.setInt(21, item.getAppearanceStoneId());
	    statement.setInt(22, item.getVisualId());
	    statement.setInt(23, item.getObjectId());
	}
