...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		else if(fullString.startsWith("admin_setsubclass"))
		{
			...
			if(st.countTokens() > 1)
			{
				...
				{
					activeChar.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2VillageMasterInstance.SubclassCouldNotBeAdded"));
					return false;
				}
		...
		else if(fullString.startsWith("admin_addrp"))
			try
			{
				String val = fullString.substring(12);
                ...
                player.addRaidPoints(points, true);
			}
			catch(StringIndexOutOfBoundsException e)
			{
				activeChar.sendMessage("Please specify new raid points value.");
			}
	...
	private void setSubclass(final Player activeChar, final Player player)
	{
		...
		{
			activeChar.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2VillageMasterInstance.NoSubAtThisTime"));
			return;
		}
