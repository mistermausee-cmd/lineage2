	...
	public boolean useItem(Playable playable, ItemInstance item, boolean ctrl)
	{
		...
		if(!rp.isCommon())
		{
			if(player.getDwarvenRecipeLimit() > 0)
			{
                ...
                if(rp.isAwakedOnly() && !player.getClassId().isAwaked())
                {
                    player.sendPacket(SystemMsg.THE_CHARACTER_AND_ITEM_RECIPE_LEVELS_DO_NOT_MATCH_SO_IT_CANNOT_BE_USED_NORMALLY);
                    return false;
                }
