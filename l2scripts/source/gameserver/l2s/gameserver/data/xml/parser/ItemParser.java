			...
			for(Iterator<org.dom4j.Element> subIterator = itemElement.elementIterator(); subIterator.hasNext();)
			{
				...
				else if(subName.equalsIgnoreCase("skills"))
				{
					for(Iterator<org.dom4j.Element> nextIterator = subElement.elementIterator(); nextIterator.hasNext();)
					{
						...
						int subLevel = nextElement.attributeValue("sub_level") == null ? 0 : Integer.parseInt(nextElement.attributeValue("sub_level"));
			            int levelMsk = SkillUtils.getSkillLevelMask(level, subLevel);

						SkillEntry skillEntry = SkillHolder.getInstance().getSkillEntry(id, levelMsk);
						...
						if(skillEntry != null)
						{
							...
							{
								if(haveNoAltSkill)
									warn("Item:" + set.getObject("item_id") + " already has a \"no-alt\" skill: ID[" + skillEntry.getId() + "] LEVEL[" + level + "] SUB_LEVEL[" + subLevel + "] that can lead to malfunction of item!");
								else
									haveNoAltSkill = true;

								if(!skillEntry.getTemplate().isHandler())
									warn("Item:" + set.getObject("item_id") + " have \"no-handler\" skill: ID[" + skillEntry.getId() + "] LEVEL[" + level + "] SUB_LEVEL[" + subLevel + "]!");
							}

							template.attachSkill(skillEntry);
						}
						else
							warn("Skill not found(" + id + "," + level + "," + subLevel + ") for item:" + set.getObject("item_id") + "; file:" + getCurrentFileName());
					}
				}
				else if(subName.equalsIgnoreCase("enchant4_skill"))
				{
					...
					int subLevel = subElement.attributeValue("sub_level") == null ? 0 : Integer.parseInt(subElement.attributeValue("sub_level"));
		            int levelMsk = SkillUtils.getSkillLevelMask(level, subLevel);
