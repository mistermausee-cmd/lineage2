...
public class StatFunctions
{
	...
	private static class FuncPAtkMul extends Func
	{
		...
		public void calc(Env env)
		{
			...
			env.value *= BaseStats.CHA.calcBonus(env.character);
		}
	}
	...
	private static class FuncHennaLUC extends Func
	{
		...
		public void calc(Env env)
		{
			Player pc = (Player) env.character;
			if(pc != null)
				env.value = Math.max(1, env.value + pc.getHennaList().getLUC());
		}
	}
	...
	private static class FuncHennaCHA extends Func
	{
		...
		public void calc(Env env)
		{
			Player pc = (Player) env.character;
			if(pc != null)
				env.value = Math.max(1, env.value + pc.getHennaList().getCHA());
		}
	}
