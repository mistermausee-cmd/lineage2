	...
	public boolean checkCondition(Creature activeChar, Creature target, boolean forceUse, boolean dontMove, boolean first)
	{
		...
		if(first)
		{
			...
			for(Creature ch : around)
			{
				...
				if(ch.isDualCastingNow() && ch.getDualCastingSkill() == this) 
				{
					activeChar.sendPacket(SystemMsg.A_FLAG_IS_ALREADY_BEING_DISPLAYED_ANOTHER_FLAG_CANNOT_BE_DISPLAYED);
					return false;
				}
		...
		if(first)
			siegeEvent.broadcastTo(new SystemMessagePacket(SystemMsg.S1_CLAN_IS_TRYING_TO_DISPLAY_A_FLAG).addString(player.getClan().getName()), CastleSiegeEvent.DEFENDERS);
