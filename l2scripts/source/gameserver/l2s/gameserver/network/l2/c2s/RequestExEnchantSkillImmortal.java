...
public final class RequestExEnchantSkillImmortal extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(!SkillUtils.isAvailableSkillEnchant(activeChar, skillEntry.getTemplate(), Skill.EnchantType.IMMORTAL))
		{
			activeChar.sendActionFailed();
			return;
		}
		...
		activeChar.sendPacket(new SystemMessage(SystemMessage.SUCCEEDED_IN_ENCHANTING_SKILL_S1).addSkillName(skillEntry.getId(), skillEntry.getLevel()), new ExEnchantSkillResult(1));
		activeChar.sendSkillList();
		activeChar.updateSkillShortcuts(skillEntry.getId(), skillEntry.getLevel());
		Log.add(activeChar.getName() + "|Successfully immortal enchanted|" + skillEntry.getId() + "|to+" + skillEntry.getLevel(), "enchant_skills");
