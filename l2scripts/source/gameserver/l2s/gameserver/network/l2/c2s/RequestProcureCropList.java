	...
	protected void runImpl()
	{
		...
		try
		{
			...
			for(int i = 0; i < _count; i++)
			{
				...
				activeChar.sendPacket(new SystemMessagePacket(SystemMsg.TRADED_S2_OF_S1_CROPS).addItemName(cropId).addLong(count), SystemMessagePacket.removeItems(cropId, count), SystemMessagePacket.obtainItems(rewardItemId, rewardItemCount, 0));
				if(fee > 0L)
					activeChar.sendPacket(new SystemMessagePacket(SystemMsg.S1_ADENA_HAS_BEEN_WITHDRAWN_TO_PAY_FOR_PURCHASING_FEES).addLong(fee));
			}
		}
		finally
		{
