...
public class RequestRecipeShopMakeDo extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
	    ...
	    if(recipe.isAwakedOnly() && !manufacturer.getClassId().isAwaked())
	    {
	    	buyer.sendActionFailed();
	    	return;
	    }
		...
		if(product != null)
		{
			...
			rate = (int)(rate + buyer.getPremiumAccount().getBonus().getCraftChance());
			rate = Math.min(100, rate);
			boolean lucky = Formulas.tryLuck(buyer);
