...
{
	private static final Logger _log = LoggerFactory.getLogger(RequestEnchantItemAttribute.class);

	private static final int ATRRIBUTE_DELAY = 1500;
	...
	protected void runImpl()
	{
		...
		if(System.currentTimeMillis() <= (activeChar.getLastAttributeItemTime() + ATRRIBUTE_DELAY))
		{
			activeChar.sendActionFailed();
			return;
		}
		...
		AttributeChanceVariation chanceVariation = AttributeStoneHolder.getInstance().getChanceVariation(attrStone.getVariation());
		if(chanceVariation == null)
		{
			_log.warn(getClass().getSimpleName() + ": Error while enchant item attribute. Cannot find chance variation ID[" + attrStone.getVariation() + " for stone ID[" + stone.getItemId() + "]!");
			activeChar.sendActionFailed();
			return;
		}

		AttributeChanceInfo chanceInfo = chanceVariation.getChanceInfo(itemToEnchant.getGrade());
		if(chanceInfo == null)
		{
			_log.warn(getClass().getSimpleName() + ": Error while enchant item attribute. Cannot find chance info in variation ID[" + attrStone.getVariation() + " for item GRADE[" + itemToEnchant.getGrade() + "]!");
			activeChar.sendActionFailed();
			return;
		}

		final int oldValue = itemToEnchant.getAttributeElementValue(element, true);
		final double chance = itemToEnchant.isWeapon() ? chanceInfo.getWeaponChance() : chanceInfo.getArmorChance();
		...
		activeChar.setLastAttributeItemTime(System.currentTimeMillis());
	}
}