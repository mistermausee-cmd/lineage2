						...
						for(int i = 0; i < ingridientItemCount * _amount; i++)
						{
							...
							if(keepenchant)
							{
								...
								for(ItemInstance item : list)
								{
									...
									if((item.getEnchantLevel() == ingridientEnchant || !item.getTemplate().isEquipment()) && !items.contains(itmd) && !item.isShadowItem() && !item.isTemporalItem() && ItemFunctions.checkIfCanDiscard(activeChar, item))
									{
										itemToTake = item;
										break;
									}
