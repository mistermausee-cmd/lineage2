	...
	private static void registerAtCastle(Player player, Castle castle, boolean attacker, boolean join)
	{
		...
		if(join)
		{
			...
			if(attacker)
			{
                ...
                if(siegeEvent.getSiegeClan(CastleSiegeEvent.ATTACKERS, playerClan) != null)
                {
                    player.sendPacket(SystemMsg.YOU_HAVE_ALREADY_REQUESTED_A_CASTLE_SIEGE);
                    return;
                }
                
                if(siegeEvent.getSiegeClan(CastleSiegeEvent.DEFENDERS, playerClan) != null || siegeEvent.getSiegeClan(CastleSiegeEvent.DEFENDERS_WAITING, playerClan) != null || siegeEvent.getSiegeClan(CastleSiegeEvent.DEFENDERS_REFUSED, playerClan) != null)
                {
                ...
                final Fortress fortress = player.getFortress();
