...
CAT_PRICE = {
    ...
    600045: 1_000_000,     600048: 5_000_000,                          # crystals / agathions
    600049: 5_000_000,     600057: 1_000_000,                          # cosmetics / quest-clan
...
# Кристаллы для крафта/заточки (D..R)
CRYSTALS = [1458,1459,1460,1461,1462,17371]
# Агатионы (браслет-петы с баффами) - курированный набор
AGATHIONS = [10018,10316,10322,10659,10660,13756,14027]
# Косметика (модные шапки, постоянные)
COSMETICS = [12782,12789,20666,20668,20672]
# Клан/Квест предметы (прокачка клана)
QUESTCLAN = [9910,9911,9912,1419]
...
def main():
    ...
    cats = {
        ...
        600045: fixed(by_ids(items, CRYSTALS), 600045),
        600046: fixed(by_ids(items, PETS), 600046),
        600047: fixed(by_ids(items, SHIRTS), 600047),
        600048: fixed(by_ids(items, AGATHIONS), 600048),
        600049: fixed(by_ids(items, COSMETICS), 600049),
        600057: fixed(by_ids(items, QUESTCLAN), 600057),
...
DEPARTMENTS = [
    ...
    ("accessory","Аксессуары",   [("Талисманы",600008),("Плащи",600025),("Пояса",600026),("Головные уборы",600034),("Рубашки",600047),("Агатионы",600048),("Петы",600046)]),
    ...
    ("craft",    "Крафт РБ",     [("Материалы",600041),("Рецепты R",600042),("Кристаллы",600045)]),
    ("grocery",  "Бакалея",      [("Расходники",600035),("Заряды (шоты)",600011),("Бустеры XP/вайт",600043)]),
    ("misc",     "Косметика/Клан",[("Косметика (шапки)",600049),("Клан/Квест",600057)]),
]
