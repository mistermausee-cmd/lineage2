...
def price_leveling(mid, pid):
    # Прокачка (B/A/S/S80) — СТРОГО по цене продажи NPC, без всякой наценки (точное reference price).
    npc = npc_price(pid)
    if npc > 0:
        return npc                              # ровно NPC-цена, без округления/наценки
    is_w = itype(pid) == "Weapon"
    return (2*M if is_w else 500_000)           # запасной вариант, если у предмета нет NPC-цены
...
def price_brooch(pid, up):
    ...
    # Понижены ещё в 3 раза относительно прошлой версии — первая брошь доступна мид-тиру.
    m = {38766:15*M, 38767:40*M, 38768:100*M, 26474:250*M,
         28358:500*M, 28486:1000*M, 28488:1300*M, 28487:1700*M, 28530:100*M}
    return sc(m.get(int(pid), 350*M))
...
# ---------- ЕДИНЫЕ ЦЕНЫ (синхрон повторов между отделами + точки/камни по характеристикам) ----------
# Значения в млн; проходят через sc() (масштаб варианта). Ключ = id предмета.
OVERRIDE = {
    37718: 500,     # Shiny Elemental Shirt («футболка Эйнхасад» — даёт активки Эйнхасада)
    # --- точки футболок: BLESSED сбрасывают до 0 (дёшево) ---
    23241: 30, 21582: 30, 25793: 30,
    # --- точки футболок: СТАБИЛЬНЫЕ (не сбрасывают/сохраняют уровень) — дороже ---
    47392: 100,     # Shining Elemental (уровень сохраняется при неудаче)
    47804: 80,      # Faint (счётчик заточки не падает)
    47805: 100,     # Сияющий Модифицировать Футболку (стабильный)
    # --- точки плащей по характеристикам ---
    28593: 20,      # Enchant Cloak +1-10 (при неудаче -1)
    28594: 40,      # Enchant Cloak Legendary +11-20 (при неудаче -1)
    28595: 60,      # Ancient Cloak Legendary +8-20, +10% успеха (при неудаче -1)
    28766: 120,     # Стабильный Legendary Плащ (уровень НЕ падает) — дороже
    # --- камни/точки хедгира и диадемы (синхрон 600034/600101) ---
    36979: 100, 45937: 100, 48215: 100, 48211: 40,
    # --- камни духа плаща (синхрон 600025/600101) ---
    28598: 150, 28599: 150, 28600: 150, 28601: 150,
    # --- лучший камень духа броши (синхрон 600090/600101; понижен вместе с брошами) ---
    28530: 100,
}
...
def compute_price(mid, pid, up, n):
    pid_i = int(pid)
    if pid_i in OVERRIDE: return sc(OVERRIDE[pid_i] * M)          # единая цена (синхрон между отделами)
    if "Gemstone" in n or "Самоцвет" in n:                        # самоцветы везде одинаково = NPC×2.5
        return round_small(npc_price(pid) * 2.5) or round_small(2500)
