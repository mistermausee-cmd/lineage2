...
## Design — Переработка основного магазина Alt+B (генератор `tools/altb_main_shop.py`)

Раздел описывает целевую сборку основного магазина Alt+B воспроизводимым скриптом
`tools/altb_main_shop.py`. Скрипт читает `Item_Catalog` (`stats/items/*.xml`) и админ-buylists
(`buylists/custom/*.xml`), затем генерирует мультиселлы `6000XX` и страницы `merchant/*.html`.
Он НЕ трогает L2Scripts-эталон (`610xxx`/`l2sref`), админ-шопы и конфиги.

### Карта отделов → категорий → multisell-id (основной магазин)

| Отдел | Категория | Multisell | Источник (buylist / id) |
|---|---|---|---|
| Прокачка | Грейд B | 600052 | MW-weapon 10995 + MW-set 10000 |
| Прокачка | Грейд A | 600053 | MW-weapon 10996 + MW-set 10001 |
| Прокачка | Грейд S | 600054 | MW-weapon 10997 + MW-set 10002 |
| Прокачка | Грейд S80 | 600055 | MW-weapon 10999(Icarus)+11000(Vesper) + MW-set 11006(Vesper) |
| R-Grade | R-Оружие (2 SA) | 600060 | normal+2SA 11027/11028/11029/11030 |
| R-Grade | Bless-оружие | 600061 | upgrade: normal(2SA) → blessed(2SA) 11011/11016/11021/11026 |
| R-Grade | PVE-оружие | 600062 | upgrade: Blessed Amaranthine → Bloody Amaranthine 11075 |
| R-Grade | R-Броня (2 SA) | 600063 | normal сеты 11031/11033/11035/11037 |
| R-Grade | Bless-броня | 600064 | upgrade: normal → blessed 11032/11034/11036/11038 |
| R-Grade | PVE-броня | 600065 | upgrade: Blessed Eternal → Bloody Eternal 35033–35047 |
| R-Grade | Запредельный ур.1–6 | 600066–600071 | upgrade-цепочка → Transcendent Eternal 28034+ (шаг уровня +45) |
| Аксессуары | Пояса | 600026 | постоянные пояса из Item_Catalog (bodypart=waist) |
| Аксессуары | Футболки | 600047 | фиксированный список + свитки модификации |

Удаляются осиротевшие мультиселлы `600001–600006` (старые R-оружие/броня) и `600050`,`600051`
(тиры D/C), а также страницы `merchant/weapons.html` и `merchant/armor.html`.

### Ключевые правила генерации

- **base_weapons(buylist):** из buylist оружия оставляет один предмет на имя (предпочитая вариант без SA,
  иначе минимальный `id`) — убирает SA/enchant-дубликаты для тиров Прокачки и категории R-Оружие.
- **Маппинг normal→blessed (оружие/броня):** по ключу `("Blessed " + name, additionalName)`; для брони —
  по `(name, additionalName)`.
- **Маппинг blessed→bloody (PVE-броня) и bloody→transcendent (ур.1):** по паре `(bodypart, weight)`,
  где weight ∈ {Heavy, Light, Robe} определяется по `additionalName`/`name`.
- **Цепочка Transcendent:** базовый id уровня = `28034 + (level-1)*45`, 15 стандартных частей
  (Heavy 5 + Light 5 + Robe 5) на уровень; ур.1 стартует от Bloody Eternal (35033–35047), далее
  ур.(N-1) → ур.N.
- **Схема upgrade-Multisell:** `maintainEnchantment="true"`, `ingredient` базового предмета (count=1) +
  `ingredient id="57"` (Adena, count=цена), один `production` (count=1).
- **Ценообразование (Anti_Exploit_Rule):** `buy = max(grade_floor, roundup(price*2))`, где `price` —
  поле цены предмета; `price*2` заведомо выше цены выкупа NPC (≤ price). Полы по грейдам:
  B=2M, A=8M, S=30M, S80=80M, R=500M, R95=800M, R99=1.5B; пояса=100M; футболки=10M.
  Стоимость upgrade (Adena): Bless-оружие=800M, PVE-оружие=2B, Bless-броня=600M, PVE-броня=1.5B,
  Transcendent ур.N=1B×N.

### Навигация

`merchant/main.html` содержит кнопку «R-Grade» (→ `rgrade.html`) вместо прежних «Оружие»/«Броня»,
кнопку «Прокачка» (→ `level.html`) и прочие отделы. `rgrade.html` содержит все 12 категорий R-Grade
напрямую (достижимость ≤ 2 клика), `level.html` — 4 тира. Все страницы используют `%navigation%`,
поэтому существующие вкладки Alt+B (Баффер/Телепорт/Поиск дропа/Понижение/Премиум) и кнопка
«Магазин L2Scripts» сохраняются без изменений. На каждой странице отдела — одна кнопка возврата
«<= В отделы».

### Валидация вывода

```
python3 tools/altb_main_shop.py --dry-run   # отчёт покрытия без записи
python3 tools/altb_main_shop.py             # генерация
xmllint --noout --schema server/game/data/xsd/multisell.xsd server/game/data/multisell/6000*.xml
```
Проверяется: well-formed + XSD; наличие `<npcs><npc>-1</npc></npcs>` и `ingredient id="57"`; резолвинг
всех `production`/`ingredient` id в `Item_Catalog`; корректность ссылок HTML на существующие страницы и мультиселлы.
